...
const sampleCompany: CompanyInfo = { name: "Anadolu Seramik Dış Ticaret Ltd. Şti.", contactPerson: "Elif Yılmaz", email: "elif.yilmaz@anadoluseramik.com", phone: "+90 532 555 24 18", address: "Organize Sanayi Bölgesi 4. Cadde No: 18, Kütahya, Türkiye", website: "https://www.anadoluseramik.com", logoUrl: "" };
const sampleBuyer: BuyerInfo = { company: "Nordic Home Trading GmbH", contactName: "Anna Müller", country: "Germany", email: "anna.mueller@nordichome.de" };
const sampleProduct: ProductInfo = { name: "Handmade Ceramic Dinnerware Set", category: "Tableware / Ceramic Products", descriptionTr: "24 parçalı, el işçiliği seramik yemek takımı. Gıda ile temasa uygun sır kaplama ve ihracata uygun korumalı ambalaj.", quantity: 250, unit: "sets", unitPrice: 38.5, currency: "USD", incoterm: "FOB", deliveryTime: "3-4 weeks after order confirmation", paymentTerms: "50% advance payment, 50% before shipment", validityDays: 15, hsCode: "6912.00", packagingDetails: "Each set is packed in a protective export carton. 20 cartons per pallet.", notes: "Product colours can be confirmed before production." };

function Field({ label, ...props }: InputHTMLAttributes<HTMLInputElement> & { label: string }) { return <label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">{label}</span><input {...props} className="min-h-12 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100" /></label>; }
function Area({ label, ...props }: TextareaHTMLAttributes<HTMLTextAreaElement> & { label: string }) { return <label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">{label}</span><textarea {...props} className="min-h-24 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 placeholder:text-slate-400 outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100" /></label>; }
function Select({ label, value, options, onChange }: { label: string; value: string; options: string[]; onChange: (value: string) => void }) { return <label className="block"><span className="mb-2 block text-sm font-bold text-slate-700">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="min-h-12 w-full rounded-xl border border-slate-300 bg-white px-4 py-3 text-sm text-slate-900 outline-none focus:border-cyan-600 focus:ring-2 focus:ring-cyan-100">{options.map((option) => <option key={option}>{option}</option>)}</select></label>; }
...
export function QuoteForm() {
  ...
  return <form onSubmit={submit} className="space-y-6">
    <div className="flex flex-col gap-3 rounded-2xl border border-cyan-200 bg-cyan-50 p-5 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-black text-slate-950">Demo için hızlı başlangıç</p><p className="mt-1 text-sm leading-6 text-slate-700">Gerçekçi örnek ihracat verilerini tek tıkla forma ekleyin.</p></div><Button type="button" variant="secondary" onClick={fillSampleData} className="shrink-0">Örnek Veri Doldur</Button></div>
    <Card className="p-5 sm:p-6"><SectionTitle number="01" title="Firma bilgileri" copy="Teklifi gönderen şirketin kurumsal bilgileri."/><div className="mt-6 grid gap-4 md:grid-cols-2"><Field label="Firma adı" placeholder="Örn. Anadolu Seramik Dış Ticaret Ltd. Şti." required {...companyField("name")}/><Field label="Yetkili kişi" placeholder="Örn. Elif Yılmaz" required {...companyField("contactPerson")}/><Field label="E-posta" type="email" placeholder="elif@firmanız.com" required {...companyField("email")}/><Field label="Telefon / WhatsApp" type="tel" placeholder="+90 5xx xxx xx xx" required {...companyField("phone")}/><Field label="Web sitesi" type="url" placeholder="https://www.firmaniz.com" required {...companyField("website")}/><Field label="Logo URL (opsiyonel)" type="url" placeholder="https://.../logo.png" {...companyField("logoUrl")}/><div className="md:col-span-2"><Area label="Adres" placeholder="Firma adresinizi yazın" required {...companyField("address")}/></div></div></Card>
