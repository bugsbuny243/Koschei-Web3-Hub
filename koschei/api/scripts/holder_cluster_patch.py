...
func summarizeHolderCluster(out HolderClusterAnalysis) HolderClusterAnalysis {
	...
	sort.SliceStable(out.SharedFundingGroups, func(i, j int) bool { return out.SharedFundingGroups[i].MemberCount > out.SharedFundingGroups[j].MemberCount })
...
func holderClusterString(raw any) string { value := strings.TrimSpace(fmt.Sprint(raw)); if value == "<nil>" { return "" }; return value }
func holderClusterFloat(raw any) float64 { switch value := raw.(type) { case float64: return value; case int: return float64(value); case int64: return float64(value) }; parsed, _ := strconv.ParseFloat(holderClusterString(raw), 64); return parsed }
func holderClusterInt64(raw any) int64 { switch value := raw.(type) { case int64: return value; case int: return int64(value); case float64: return int64(value) }; parsed, _ := strconv.ParseInt(holderClusterString(raw), 10, 64); return parsed }
func holderClusterRound(value float64, digits int) float64 { factor := math.Pow10(digits); return math.Round(value*factor)/factor }
func holderClusterUnixTime(value int64) string { if value <= 0 { return "" }; return time.Unix(value, 0).UTC().Format(time.RFC3339) }
func compactClusterError(err error) string { if err == nil { return "" }; value := strings.TrimSpace(err.Error()); if len(value) > 180 { value = value[:180] }; return value }
'''

service_test = r'''package services

import "testing"

func TestSummarizeHolderClusterHighConfidence(t *testing.T) {
	wallets := []HolderClusterWallet{
		{Wallet: "A", HolderPercentage: 4, Status: "verified_bounded_observation", FreshNearLaunch: true, FundingSource: "F", FundingAmountSOL: .1, AcquisitionSlot: 100},
		{Wallet: "B", HolderPercentage: 3, Status: "verified_bounded_observation", FreshNearLaunch: true, FundingSource: "F", FundingAmountSOL: .1, AcquisitionSlot: 101},
		{Wallet: "C", HolderPercentage: 2, Status: "verified_bounded_observation", FreshNearLaunch: true, FundingSource: "F", FundingAmountSOL: .1, AcquisitionSlot: 102},
	}
...
Path('internal/services/holder_cluster_intelligence.go').write_text(service)
Path('internal/services/holder_cluster_intelligence_test.go').write_text(service_test)

p = Path('internal/services/security_radars.go')
s = p.read_text()
s = s.replace('''\tHolderRoles            HolderRoleAnalysis
\tRecentSignatureCount   int''', '''\tHolderRoles            HolderRoleAnalysis
\tHolderCluster          HolderClusterAnalysis
\tTargetOldestBlockTime  int64
\tTargetOldestSlot       int64
\tRecentSignatureCount   int''', 1)
s = s.replace('''\tctx, cancel := context.WithTimeout(context.Background(), 6500*time.Millisecond)
\tdefer cancel()''', '''\ttimeout := 6500 * time.Millisecond
\tif strings.Contains(strings.ToLower(req.Mode), "owner") || strings.Contains(strings.ToLower(req.Mode), "manual") {
\t\ttimeout = 18 * time.Second
\t}
\tctx, cancel := context.WithTimeout(context.Background(), timeout)
\tdefer cancel()''', 1)
s = s.replace('''\t\t\t\tif oldest == 0 || *sig.BlockTime < oldest {
\t\t\t\t\toldest = *sig.BlockTime
\t\t\t\t}''', '''\t\t\t\tif oldest == 0 || *sig.BlockTime < oldest {
\t\t\t\t\toldest = *sig.BlockTime
\t\t\t\t\tprofile.TargetOldestSlot = sig.Slot
\t\t\t\t}''', 1)
s = s.replace('''\t\tif newest > 0 && oldest > 0 && newest >= oldest {
\t\t\tprofile.SignatureWindowSeconds = newest - oldest
\t\t}
\t} else {''', '''\t\tif newest > 0 && oldest > 0 && newest >= oldest {
\t\t\tprofile.SignatureWindowSeconds = newest - oldest
\t\t\tprofile.TargetOldestBlockTime = oldest
\t\t}
\t} else {''', 1)
s = s.replace('''\tif profile.LiveRPC {
\t\tprofile.DataQuality = "live_rpc_evidence"''', '''\tif profile.IsTokenMint && profile.HolderRoles.Available {
...
\tif profile.LiveRPC {
\t\tprofile.DataQuality = "live_rpc_evidence"''', 1)
p.write_text(s)

p = Path('internal/services/arvis_arms.go')
s = p.read_text()
s = s.replace('''\tfunding := unavailableArm("Funding Cluster Detector", ModuleFundingClusterDetector, req, generatedAt, "Parsed funding transactions and counterparty graph evidence are required.")''', '''\tfunding := buildFundingClusterArm(req, profile, generatedAt)''', 1)
s = s.replace('''\t\t\t"data_quality": profile.DataQuality, "evidence_status": profile.EvidenceStatus,
\t\t},''', '''\t\t\t"data_quality": profile.DataQuality, "evidence_status": profile.EvidenceStatus,
\t\t\t"holder_cluster_analysis": profile.HolderCluster,
\t\t},''', 1)
pattern = re.compile(r'func buildSniperTimingArm\(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string\) SecurityRadarVerdict \{.*?\n\}\n\nfunc buildProgramRelationArm', re.S)
replacement = r'''func buildFundingClusterArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	a := p.HolderCluster
	if !a.Available {
...
func buildProgramRelationArm'''
if not pattern.search(s):
    raise SystemExit('sniper timing block not found')
s = pattern.sub(replacement, s, count=1)
p.write_text(s)

p = Path('internal/handlers/owner_operations.go')
s = p.read_text()
s = s.replace('''\t\t"modules": modules, "evidence": evidence, "graph": graph,
\t\t"evidence_policy": map[string]any{''', '''\t\t"modules": modules, "evidence": evidence, "graph": graph,
\t\t"holder_cluster": ownerRadarModuleSignal(modules, services.ModuleFundingClusterDetector, "holder_cluster_analysis"),
\t\t"evidence_policy": map[string]any{''', 1)
insert = r'''
...
'''
marker = 'func ownerRadarNarrative('
if marker not in s:
    raise SystemExit('owner narrative marker not found')
