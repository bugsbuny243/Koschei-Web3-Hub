(()=>{
  ...
  const truth=window.KoscheiFullScanTruthContract;
  ...
  const moduleExecution=module=>truth?.moduleExecution?.(module)||'evidence_pending';
  ...
  function renderCapabilities(payload){
    const legacy=obj(payload.legacy_14_arm_radar),coverage=obj(legacy.investigation_coverage),market=obj(legacy.market),holder=obj(legacy.holder_intelligence);
    const classified=truth?.classifyModules?.(payload)||{completed:[],gaps:[]},complete=classified.completed,gaps=classified.gaps;
    if(!complete.length&&!gaps.length)return'';
    const facts=complete.map(module=>{const detail=truth?.moduleTechnicalDetail?.(module)||'';return`<div class="summary-row"><span class="mono">${esc(module.module_id||'collector')}</span><b style="text-align:left">${esc(module.module||module.module_id)}${detail?`<small class="muted">${esc(short(detail,120))}</small>`:''}</b>${badge(module.verified?'verified':'completed',module.verified?'DOĞRULANDI':'TAMAMLANDI')}</div>`}).join('');
  ...
  function decorate(root,payload){
    ...
    if(truth?.actorDossierEmpty?.(payload)){
