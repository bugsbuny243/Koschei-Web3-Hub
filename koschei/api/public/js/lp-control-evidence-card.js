(()=>{
  ...
  function movementRows(lp,lang){
    return arr(lp.liquidity_movements).map(row=>`<tr><td><b>${esc(statusLabel(row.kind,lang))}</b><small>${esc(text(row.block_time)||'—')}</small></td><td>${copy(row.actor_wallet)}</td><td>${esc(number(row.token_delta))}</td><td>${esc(number(row.quote_delta))}</td><td>${copy(row.signature)}<small>slot ${esc(row.slot||'—')}</small></td></tr>`).join('');
  }
