...
{{with .Resolution}}<div class="result" role="status"><h2>{{.Network.Name}} address format recognized</h2><p>{{.CanonicalRef}}</p><p>Collector implementation: {{.Network.CollectorStatus}}. Deployment and live availability have not been checked. No analysis has been performed; evidence remains unknown.</p>{{if eq .Network.Family "evm"}}<p>Hex format was checked. Address checksum, account existence, account type and ownership were not verified.</p>{{end}}</div>{{end}}</section>
<section class="panel table"><h2>Implementation coverage</h2><table><thead><tr><th>Network</th><th>Address format</th><th>Collector</th></tr></thead><tbody>{{range .Networks}}<tr><td>{{.Name}}</td><td>{{.AddressFormat}}</td><td>{{if eq .CollectorStatus "existing"}}Existing collector; live health not checked{{else if eq .CollectorStatus "probe_ready"}}Probe implemented; deployment and live health not checked{{else}}Implementation not connected{{end}}</td></tr>{{end}}</tbody></table><p class="muted">A shared address format does not prove an account exists on another network. Probe readiness describes implemented code only; deployment configuration and live evidence are reported separately by the deployment catalog and live probe endpoints.</p></section>
</body></html>`))
