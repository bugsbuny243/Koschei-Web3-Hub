...
func registerCoreRoutes(mux *http.ServeMux, h *handlers.Handler) {
	...
	mux.HandleFunc("/api/guard/check", method("POST", h.GuardCheck))
...
func registerRuntimeRoutes(mux *http.ServeMux, h *handlers.Handler) {
...
func registerCustomerProductRoutes(mux *http.ServeMux, h *handlers.Handler, premium func(http.HandlerFunc) http.HandlerFunc) {
...
func registerB2BRoutes(mux *http.ServeMux, h *handlers.Handler) {
	...
	mux.HandleFunc("/api/v1/radar/graph", requiresDB(h, handlers.RequireAuth(h.RequireActiveEntitlement(method("GET", h.SecurityRadarGraph)))))
	mux.HandleFunc("/api/v1/shield/preflight", requiresDB(h, h.APIKeyAuth(h.APIRateLimit(method("POST", h.ShieldPreflight)))))
	mux.HandleFunc("/api/v1/shield/transaction", requiresDB(h, h.APIKeyAuth(h.APIRateLimit(method("POST", h.ShieldPreflight)))))
...
func registerWeb3Routes(mux *http.ServeMux, h *handlers.Handler, premium func(http.HandlerFunc) http.HandlerFunc) {
...
func registerStaticFiles(mux *http.ServeMux, staticDir string) {
	if staticDir == "" {
		return
	}
