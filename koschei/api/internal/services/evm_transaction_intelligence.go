...
func AdaptEVMTransactionEvidence(result networktarget.EVMTransactionEvidenceResult, observedAt time.Time) (NetworkProbeIntelligenceProjection, error) {
	...
	attrs := map[string]any{
		...
		"input_selector":       result.InputSelector,
		"input_selector_hint":  result.InputSelectorHint,
		"selector_hint_only":   result.InputSelectorHint != "",
		"standard_event_count": result.StandardEventCount,
		"transfer_event_count": result.TransferEventCount,
		"approval_event_count": result.ApprovalEventCount,
	...
	if len(result.Logs) > 0 {
		...
		for _, item := range result.Logs {
			logs = append(logs, map[string]any{
				...
				"semantic_layout": item.SemanticLayout,
