...
func AnalyzeArvisRadars(req SecurityRadarRequest) ArvisAnalysis {
	...
	if req.Network == "" { req.Network = "solana-mainnet" }
	if req.Mode == "" { req.Mode = SecurityRadarWatchMode }

	generatedAt := time.Now().UTC().Format(time.RFC3339)
	profile := collectRadarEvidence(req)

	pump := unavailableArm("Pump.fun Sybil Radar", ModulePumpSybilRadar, req, generatedAt, "Launch, first-buyer and funding evidence is unavailable.")
	if profile.LiveRPC && (profile.RecentSignatureCount > 0 || profile.LargestAccounts > 0) {
		pump = buildPumpSybilVerdict(req, profile, generatedAt)
	}

	raydium := unavailableArm("Raydium Pool Guardian", ModuleRaydiumPoolGuardian, req, generatedAt, "Pool or token account evidence is unavailable.")
	if profile.LiveRPC && profile.AccountExists {
	...
	for _, arm := range arms[:len(arms)-1] {
		if arm.Signed && arm.Signals != nil {
			if ok, _ := arm.Signals["real_onchain_evidence"].(bool); ok { verified++ }
		}
	}
	...
	bundle := SecurityRadarBundle{
		...
		Metadata: map[string]any{
			...
			"runtime_arm_count": verified, "arvis_arms": arms,
...
func ArvisArmsFromBundle(bundle SecurityRadarBundle) []SecurityRadarVerdict {
	if bundle.Metadata == nil { return nil }
	...
	case []any:
		...
		for _, item := range raw {
			if verdict, ok := item.(SecurityRadarVerdict); ok { out = append(out, verdict) }
		}
		return out
	default:
		return nil
...
func ArvisFinalFromBundle(bundle SecurityRadarBundle) SecurityRadarFinalVerdict {
	for _, arm := range ArvisArmsFromBundle(bundle) {
		if arm.ModuleID == ModuleFinalVerdictEngine { return finalVerdictFromArm(arm) }
	}
...
func buildIntelligenceGraphArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	...
	if p.FailedSignatureCount >= 10 { risk += 8 }
	s := armSignals(p, ModuleIntelligenceGraph)
	s["account_owner"] = p.AccountOwner; s["latest_signature"] = p.LatestSignature; s["largest_accounts"] = p.LargestAccounts
...
func buildAuthorityArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	...
	risk := 5
	if p.MintAuthorityPresent { risk += 38 }
	if p.FreezeAuthorityPresent { risk += 38 }
	s := armSignals(p, ModuleTokenAuthorityScanner)
	s["mint_authority_present"] = p.MintAuthorityPresent; s["freeze_authority_present"] = p.FreezeAuthorityPresent; s["account_owner"] = p.AccountOwner
...
func buildHolderArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	...
	s := armSignals(p, ModuleHolderConcentration)
	s["largest_holder_percentage"] = p.LargestHolderPct; s["top_10_holder_percentage"] = p.Top10HolderPct; s["largest_accounts"] = p.LargestAccounts; s["token_supply"] = p.TokenSupply
...
func buildSniperTimingArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	...
	s := armSignals(p, ModuleSniperTimingDetector)
	s["recent_signature_count"] = p.RecentSignatureCount; s["signature_window_seconds"] = p.SignatureWindowSeconds; s["failed_signature_count"] = p.FailedSignatureCount
...
func buildProgramRelationArm(req SecurityRadarRequest, p radarEvidenceProfile, generatedAt string) SecurityRadarVerdict {
	...
	risk := 6
	if p.AccountExecutable { risk += 22 }
	if !p.IsTokenMint { risk += 8 }
	s := armSignals(p, ModuleProgramRelationScan)
	s["account_owner"] = p.AccountOwner; s["account_executable"] = p.AccountExecutable; s["is_token_mint"] = p.IsTokenMint
...
func buildFinalArm(req SecurityRadarRequest, arms []SecurityRadarVerdict, generatedAt string) SecurityRadarVerdict {
	...
	for _, arm := range arms {
		if !arm.Signed || arm.Signals == nil { continue }
		if ok, _ := arm.Signals["real_onchain_evidence"].(bool); ok { verified = append(verified, arm) }
	}
	if len(verified) == 0 {
...
func armSignals(p radarEvidenceProfile, moduleID string) map[string]any {
...
func unavailableArm(module, moduleID string, req SecurityRadarRequest, generatedAt, reason string) SecurityRadarVerdict {
	return SecurityRadarVerdict{Module: module, ModuleID: moduleID, Target: req.Target, Network: req.Network, Grade: "-", RiskIndex: 0, RiskLevel: "unknown", Verdict: SecurityRadarInsufficientEvidenceMessage, Recommendation: "insufficient_evidence", Signals: map[string]any{"module_id": moduleID, "real_onchain_evidence": false, "arm_evidence_available": false, "evidence_status": "insufficient_evidence", "score_source": "none"}, Evidence: []string{reason}, GeneratedAt: generatedAt, RuleVersion: SecurityRadarRuleVersion, Signed: false}
}
