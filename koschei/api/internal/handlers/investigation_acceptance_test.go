...
func acceptanceFixture(completed, producing int) map[string]any {
	...
	report["full_scan_live_evidence"] = map[string]any{
		...
		"mint": "Mint111",
		"wallet_coverage": []any{
			map[string]any{"wallet": "Creator111", "role": "creator_source_observed", "status": "completed", "parsed_transaction_count": 4},
			map[string]any{"wallet": "Owner111", "role": "risk_bearing_holder", "status": "no_relevant_mint_activity", "parsed_transaction_count": 3},
		},
		"transactions": []any{
			map[string]any{
...
func TestProductionAcceptanceProfiles(t *testing.T) {
	...
	if len(cases) != 10 { t.Fatalf("acceptance archetypes=%d", len(cases)) }
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !result.CallerParity.Passed { t.Fatalf("parity=%#v", result.CallerParity) }
...
func TestProductionAcceptanceParityHashIsStable(t *testing.T) {
	...
	if !first.Passed || !second.Passed { t.Fatalf("parity failed: %#v %#v", first, second) }
...
func assertAcceptanceBlocker(t *testing.T, result InvestigationAcceptanceResult, code string) {
	t.Helper()
	if result.Status != "fail" { t.Fatalf("status=%s blockers=%#v", result.Status, result.Blockers) }
	for _, item := range result.Blockers {
		if strings.EqualFold(item.Code, code) { return }
	}
