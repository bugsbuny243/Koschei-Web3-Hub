...
func (h *Handler) GetJobs(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() { var id,tool,prompt,route,provider,status string; var result *string; var created,updated string; if err:=rows.Scan(&id,&tool,&prompt,&route,&provider,&status,&result,&created,&updated); err!=nil { writeJSON(w,500,map[string]string{"error":"db failed"}); return }; jobs=append(jobs,map[string]any{"id":id,"tool":tool,"prompt":prompt,"route":route,"provider":provider,"status":status,"result":result,"created_at":created,"updated_at":updated}) }
...
func (h *Handler) CreateJob(w http.ResponseWriter, r *http.Request) {
	...
	if err:=h.DB.QueryRow(`INSERT INTO generation_jobs (email, tool, prompt, route, provider, status) VALUES ($1,$2,$3,$4,$5,'queued') RETURNING id, status`, req.Email, req.Tool, req.Prompt, route.Route, route.Provider).Scan(&id,&status); err!=nil { writeJSON(w,500,map[string]string{"error":"db failed"}); return }
...
type runtimeRouteRequest struct { Tool string `json:"tool"`; Prompt string `json:"prompt"` }
