...
const (
	courtTogetherEndpoint = "https://api.together.xyz/v1/chat/completions"
...
// NewCourtNarrativeClientFromEnv wires the live court only when the global
// court flag and the Together key are present. Missing frontier-provider keys
// do not disable the lower court; they only make the senior panel partial.
func NewCourtNarrativeClientFromEnv() CourtNarrativeClient {
	...
	return &liveCourtClient{
		httpClient: &http.Client{Timeout: courtEnvDuration("KOSCHEI_COURT_HTTP_TIMEOUT", 70*time.Second)},
		togetherKey: togetherKey,
		openAIKey: strings.TrimSpace(os.Getenv("OPENAI_API_KEY")),
		anthropicKey: strings.TrimSpace(os.Getenv("ANTHROPIC_API_KEY")),
		togetherEndpoint: firstNonEmptyString(strings.TrimSpace(os.Getenv("TOGETHER_BASE_URL")), courtTogetherEndpoint),
		...
		timeout: courtEnvDuration("KOSCHEI_COURT_MODEL_TIMEOUT", 55*time.Second),
...
func (c *liveCourtClient) ProsecutorOpinion(ctx context.Context, in CourtReadOnlyInput, role string) (CourtOpinion, error) {
	...
	roleTitle := "Başsavcı"
	roleInstruction := "Ana iddianameyi hazırla. Suçlayıcı ve aklayıcı delilleri birlikte tart; açık iddiaları evidence_id ile bağla."
	if strings.EqualFold(strings.TrimSpace(role), "minimax-m3") {
		model = c.prosecutorEvidenceModel
		roleTitle = "Bağımsız Delil Savcısı"
		roleInstruction = "Ana savcıdan bağımsız incele. Kanıt boşluklarını, yanlış-pozitif ihtimallerini ve masum alternatif açıklamaları özellikle ara."
	}
	system := courtSystemPrompt(roleTitle)
	prompt, err := courtPrompt(in, roleInstruction, nil, nil)
	if err != nil {
		return CourtOpinion{}, err
	}
	out, err := c.callTogether(ctx, model, system, prompt, 1700)
	if err != nil {
		return CourtOpinion{}, err
	}
...
func (c *liveCourtClient) PanelOpinion(ctx context.Context, in CourtReadOnlyInput, prosecutors []CourtOpinion) (CourtPanel, error) {
	system := courtSystemPrompt("İlk Derece Mahkemesi Üyesi")
	instruction := "Savcı görüşlerini çapraz sorgula. Yalnız dosyadaki doğrulanmış evidence_id değerlerine dayan. İddia, çıkarım ve eksik delili ayır. Signed verdict değiştirilemez."
	prompt, err := courtPrompt(in, instruction, prosecutors, nil)
	if err != nil {
		return CourtPanel{}, err
	}
	results := make(chan courtProviderResult, 2)
	var wg sync.WaitGroup
	for _, model := range []string{c.panelQwenModel, c.panelGLMModel} {
		model := model
		wg.Add(1)
		go func() {
			defer wg.Done()
			out, callErr := c.callTogether(ctx, model, system, prompt, 1900)
			if callErr != nil {
				results <- courtProviderResult{err: fmt.Errorf("%s: %w", model, callErr)}
				return
			}
	...
	if len(opinions) == 0 {
		return CourtPanel{}, errors.New("first-instance panel providers unavailable: " + strings.Join(errs, "; "))
	}
	stance := presidingCourtStance(in.SignedVerdict, opinions)
	return CourtPanel{
		Models: courtOpinionModels(opinions),
		Stance: stance,
		Text: courtPanelText("İlk Derece Heyeti", opinions, errs, in),
		Opinions: opinions,
		Limitations: errs,
	}, nil
}
...
func (c *liveCourtClient) SeniorOpinion(ctx context.Context, in CourtReadOnlyInput, prosecutors []CourtOpinion, panel *CourtPanel) (CourtPanel, error) {
	instruction := "Üst heyet incelemesi yap. Çoğunluk görüşünü, varsa muhalefet şerhini, olgu/çıkarım/sınırlama ayrımını ve deterministik görsel exhibit planını açıkla. Signed verdict nihai otoritedir; yeni delil veya skor üretme."
	prompt, err := courtPrompt(in, instruction, prosecutors, panel)
	if err != nil {
		return CourtPanel{}, err
	}
	...
	if c.openAIKey != "" && c.openAIModel != "" {
		calls = append(calls, seniorCall{provider: "openai", model: c.openAIModel, call: c.callOpenAI})
	}
	if c.anthropicKey != "" && c.anthropicModel != "" {
		calls = append(calls, seniorCall{provider: "anthropic", model: c.anthropicModel, call: c.callAnthropic})
	}
	if len(calls) == 0 {
		return CourtPanel{}, errors.New("senior court requires OPENAI_MODEL_TRIBUNAL or ANTHROPIC_OWNER_MODEL with matching API key")
	}
	...
	for _, call := range calls {
		...
		go func() {
			defer wg.Done()
			out, callErr := call.call(ctx, call.model, courtSystemPrompt("Üst Heyet Üyesi"), prompt, 2400)
			if callErr != nil {
				results <- courtProviderResult{err: fmt.Errorf("%s/%s: %w", call.provider, call.model, callErr)}
				return
			}
			results <- courtProviderResult{opinion: courtOpinionFromOutput(call.provider, call.model, out)}
		}()
	}
	wg.Wait()
	close(results)
	...
	if len(opinions) == 0 {
		return CourtPanel{}, errors.New("senior court providers unavailable: " + strings.Join(errs, "; "))
	}
	return CourtPanel{
		Models: courtOpinionModels(opinions),
		Stance: presidingCourtStance(in.SignedVerdict, opinions),
		Text: courtPanelText("Üst Heyet", opinions, errs, in),
		Opinions: opinions,
		Limitations: errs,
	}, nil
}
...
func (c *liveCourtClient) callTogether(ctx context.Context, model, system, prompt string, maxTokens int) (courtModelOutput, error) {
	if strings.TrimSpace(c.togetherKey) == "" {
		return courtModelOutput{}, errors.New("TOGETHER_API_KEY is not configured")
	}
	payload := courtChatPayload{
		Model: model,
	...
	data, err := c.doJSON(ctx, c.togetherEndpoint, body, map[string]string{
...
func (c *liveCourtClient) callOpenAI(ctx context.Context, model, system, prompt string, maxTokens int) (courtModelOutput, error) {
	if c.openAIKey == "" || strings.TrimSpace(model) == "" {
		return courtModelOutput{}, errors.New("OpenAI court configuration is incomplete")
	}
	...
	endpoint := courtEndpoint(c.openAIBaseURL, "chat/completions")
	data, err := c.doJSON(ctx, endpoint, body, map[string]string{
...
func (c *liveCourtClient) callAnthropic(ctx context.Context, model, system, prompt string, maxTokens int) (courtModelOutput, error) {
	if c.anthropicKey == "" || strings.TrimSpace(model) == "" {
		return courtModelOutput{}, errors.New("Anthropic court configuration is incomplete")
	}
	...
	if len(packet) > courtMaxEvidenceBytes {
		compact := CourtReadOnlyInput{
			...
			EvidencePacket: map[string]any{
				"truncated": true,
				"reason": "evidence packet exceeded court transport limit",
			},
		}
...
- Yalnız dosyada bulunan evidence_id, rule_id, signature ve doğrulanmış alanlara dayan.
- INFERRED/UNVERIFIED bulguları açıkça işaretle; bunları kesin olguya dönüştürme.
- Signed deterministic verdict nihai otoritedir; grade, verdict veya signature değiştirme.
...
func courtSystemPrompt(role string) string {
	return "Sen Koschei ARVIS " + role + " rolündesin. Görevin kanıt dosyasını açıklamak ve çapraz denetlemektir. Zincir gerçeği yalnız imzalı deterministik kanıttan gelir. Yeni kanıt, sayı, kimlik, niyet, grade veya verdict uyduramazsın."
}
...
func decodeCourtOutput(content string) (courtModelOutput, error) {
	...
	if content == "" {
		return courtModelOutput{}, errors.New("provider returned empty court output")
	}
...
func presidingCourtStance(verdict interface{ GetGrade() string }, opinions []CourtOpinion) string {
	// This overload is intentionally not used; kept out by concrete helper below.
	return "insufficient"
}

func presidingCourtStanceFromGrade(grade string, triggered bool, opinions []CourtOpinion) string {
	if len(opinions) == 0 {
		return "insufficient"
	}
...
func courtPanelText(title string, opinions []CourtOpinion, errors []string, in CourtReadOnlyInput) string {
	parts := []string{title + ":"}
	for _, opinion := range opinions {
		parts = append(parts, fmt.Sprintf("\n[%s / %s / %s]\n%s", opinion.Provider, opinion.Model, opinion.Stance, opinion.Text))
	}
	if len(errors) > 0 {
		parts = append(parts, "\nProvider sinirlamalari: "+strings.Join(errors, " | "))
	}
