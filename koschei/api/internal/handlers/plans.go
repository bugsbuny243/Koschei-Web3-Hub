...
func (h *Handler) Plans(w http.ResponseWriter, _ *http.Request) {
	...
	if err != nil { writeJSON(w, 500, map[string]string{"error":"failed"}); return }
	defer rows.Close()
	var out []map[string]any
	for rows.Next() {
		var id, name string; var price, credits int; var active bool
		if err := rows.Scan(&id,&name,&price,&credits,&active); err != nil { writeJSON(w,500,map[string]string{"error":"failed"}); return }
