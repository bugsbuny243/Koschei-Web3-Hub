...
func TestPostBetterAuthEmailPasswordSignUpFallbackTriesAPIRouteAfter404(t *testing.T) {
	oldClient := authProviderHTTPClient
	defer func() { authProviderHTTPClient = oldClient }()
	...
	authProviderHTTPClient = roundTripFunc(func(r *http.Request) (*http.Response, error) {
		gotURLs = append(gotURLs, r.URL.String())
		body, _ := io.ReadAll(r.Body)
		if !strings.Contains(string(body), `"email":"new@example.com"`) || !strings.Contains(string(body), `"password":"secret-password"`) {
			t.Fatalf("request body did not include email/password: %s", string(body))
		}
		switch r.URL.String() {
		case "https://auth.example.test/sign-up/email":
			return &http.Response{StatusCode: http.StatusNotFound, Body: io.NopCloser(strings.NewReader(`{"message":"not found"}`)), Header: make(http.Header)}, nil
		case "https://auth.example.test/api/auth/sign-up/email":
			return &http.Response{StatusCode: http.StatusOK, Body: io.NopCloser(strings.NewReader(`{"user":{"email":"new@example.com"}}`)), Header: make(http.Header)}, nil
		default:
			t.Fatalf("unexpected fallback URL: %s", r.URL.String())
			return nil, nil
		}
	...
	baseURL, err := postBetterAuthEmailPasswordSignUpWithFallback(
		context.Background(),
		betterAuthConfig{BaseURL: "https://auth.example.test", IssuerURL: "https://auth.example.test", JWKSURL: "https://auth.example.test/api/auth/jwks"},
		map[string]string{"email": "new@example.com", "password": "secret-password"},
	)
	if err != nil {
		t.Fatalf("postBetterAuthEmailPasswordSignUpWithFallback returned error: %v", err)
	}
	if baseURL != "https://auth.example.test/api/auth" {
		t.Fatalf("baseURL = %q, want API auth base", baseURL)
	}
	want := []string{"https://auth.example.test/sign-up/email", "https://auth.example.test/api/auth/sign-up/email"}
	if strings.Join(gotURLs, "\n") != strings.Join(want, "\n") {
		t.Fatalf("attempted URLs = %#v, want %#v", gotURLs, want)
	}
}

func TestRegisterRequiresEmailPasswordAuthEnv(t *testing.T) {
	t.Setenv("NEON_AUTH_BASE_URL", "")
	t.Setenv("NEON_AUTH_ISSUER", "")
	t.Setenv("NEON_AUTH_JWKS_URL", "")
	req := httptest.NewRequest(http.MethodPost, "/api/auth/register", strings.NewReader(`{"email":"user@example.com","password":"secret-password"}`))
	w := httptest.NewRecorder()
	...
	if !strings.Contains(w.Body.String(), "auth_not_configured") {
		t.Fatalf("response did not explain auth config issue: %s", w.Body.String())
	}
	for _, name := range []string{"NEON_AUTH_BASE_URL", "NEON_AUTH_ISSUER", "NEON_AUTH_JWKS_URL"} {
		if !strings.Contains(w.Body.String(), name) {
			t.Fatalf("response did not explain missing %s env: %s", name, w.Body.String())
...
func TestPublicRegisterAuthProviderErrorMessages(t *testing.T) {
	alreadyErr := authProviderHTTPError{StatusCode: http.StatusConflict, Body: `{"message":"User already exists"}`}
	if got := registerAuthProviderStatusCode(alreadyErr); got != http.StatusConflict {
		t.Fatalf("already exists status = %d, want %d", got, http.StatusConflict)
	}
	if got := registerAuthProviderErrorCode(alreadyErr); got != "email_already_exists" {
		t.Fatalf("already exists code = %q", got)
	}
	if got := publicRegisterAuthProviderError(alreadyErr); got != "Bu e-posta zaten kayıtlı. Giriş yapmayı deneyin." {
		t.Fatalf("already exists message = %q", got)
	}
	missingErr := authProviderHTTPError{StatusCode: http.StatusNotFound}
	if got := registerAuthProviderErrorCode(missingErr); got != "signup_endpoint_not_found" {
		t.Fatalf("missing endpoint code = %q", got)
	}
	if got := publicRegisterAuthProviderError(missingErr); got != "Neon Auth sign-up endpoint bulunamadı. NEON_AUTH_BASE_URL kontrol edilmeli." {
		t.Fatalf("missing endpoint message = %q", got)
	}
}
