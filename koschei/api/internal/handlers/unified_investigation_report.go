...
func (h *Handler) assembleUnifiedInvestigationReportMode(ctx context.Context, core holderIntelligenceCoreResult, mode string) unifiedInvestigationAssembly {
	...
	if liveRequested {
		...
		case store == nil:
			...
			actorRun.LiveEvidence = h.collectActorDefenseLiveEvidence(ctx, nil, actorDossier)
			actorDossier, actorTrack = hydrateRequestScopeActorDossier(
				actorDossier, actorTrack, externalDiscovery, actorRun.FundingOrigin,
				actorRun.FundingOriginPersistence, actorRun.LiveEvidence, network,
			)
	...
	actorLifecycle = applyRequestScopeActorLifecycleRecurrence(&core, actorLifecycle, externalDiscovery, actorDossier, creator, network, target)
