...
// the technical result. Operational metadata and persistence are added by the
// caller outside Report.
...
func (h *Handler) assembleUnifiedInvestigationReport(ctx context.Context, core holderIntelligenceCoreResult) unifiedInvestigationAssembly {
	...
	// Phase 1 is a stored-data pipe repair. It must not create new RPC traffic.
	// Candidate signatures stay visible, while transaction-level verification is
	// added by later collection stages without changing the stored-ledger facts.
...
func (h *Handler) unifiedTradeLedgerAggregates(ctx context.Context, mint string) map[string]any {
	...
	out["available"] = tradeCount > 0
	out["status"] = "observed_trade_ledger_aggregates"
	out["trade_count"] = tradeCount
	out["buy_count"] = buyCount
	out["sell_count"] = sellCount
	out["unique_trader_count"] = uniqueTraders
