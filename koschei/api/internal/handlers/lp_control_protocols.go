...
func collectProtocolLPControlEvidence(ctx context.Context, rpc solanaRPCCall, network, mint, creator, pool string) services.LPControlEvidence {
	...
	case meteoraDLMMProgram:
		...
		out.Available = true
		out.ReasonCode = "meteora_dlmm_layout_not_decoded"
		out.EvidenceKeys = append(out.EvidenceKeys, fmt.Sprintf("pool:%s@%d", out.PoolAddress, out.ReadSlot))
		out.Limitations = append(out.Limitations, "The pool is owned by the pinned Meteora DLMM program. Position ownership and reserve offsets are withheld until the exact deployed account layout is decoded.")
		return out
	default:
...
func decodePumpSwapLPControl(ctx context.Context, rpc solanaRPCCall, network, creator string, out services.LPControlEvidence, data []byte) services.LPControlEvidence {
	...
	poolCirculatingLP := float64(binary.LittleEndian.Uint64(data[203:211]))
	out = populateDecodedLPControl(ctx, rpc, network, creator, out)
	out.LPSupplySource = "mint_supply"
	out.EvidenceKeys = append(out.EvidenceKeys, fmt.Sprintf("pumpswap_pool_index:%d", index), fmt.Sprintf("pumpswap_circulating_lp_raw:%.0f", poolCirculatingLP))
	if len(data) >= 261 {
