...
| Smart Money product surface | Snapshot, stream and watchlist routes had no verified whale/CEX-flow enrichment source. | Smart Money API, stream and redirect page were retired instead of advertising an unavailable product. | `koschei/api/internal/http/server.go`, retired `smart_money.go` and `smart-money.html` |
...
| Unimplemented auth and AI routes | OTP and generic AI endpoints returned `501 Not Implemented`; unused AI media handlers were not connected to a production provider. | Routes and unused handlers were removed from the public server surface. | `koschei/api/internal/http/server.go`, `koschei/api/internal/handlers/route_stubs.go`, retired `ai_media.go` |
| Unrelated game tooling | Owner game studio, customer game generation and Android build routes were unrelated to the Solana risk-intelligence product. | Public route exposure was removed; internal code remains quarantined pending a separate dependency-safe deletion. | `koschei/api/internal/http/server.go` |
