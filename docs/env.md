...
- EXPO_PUBLIC_ENABLE_MEDIA_MODULES=false
- TOGETHER_MODEL_SECURITY=
- TOGETHER_SECURITY_TIMEOUT_SECONDS=120
- TOGETHER_SECURITY_MAX_TOKENS=2500


- `TOGETHER_MODEL_SECURITY` is used by Cyber Defense Center.
- Cyber model fallback order: `TOGETHER_MODEL_SECURITY` -> `TOGETHER_MODEL_REASONING` -> `TOGETHER_MODEL_COMPLEX` -> `TOGETHER_MODEL`.
