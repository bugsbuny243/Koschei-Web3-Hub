...
UPDATE auth_accounts SET plan_id=COALESCE(NULLIF(plan,''),'free') WHERE plan_id IS NULL OR plan_id='';
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'auth_accounts_email_key'
  ) THEN
    ALTER TABLE auth_accounts ADD CONSTRAINT auth_accounts_email_key UNIQUE (email);
  END IF;
END $$;
