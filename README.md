...
# Go auth-api Neon Auth and JWT validation settings:
NEON_AUTH_BASE_URL=...
NEON_AUTH_ISSUER=...
NEON_AUTH_JWKS_URL=...
...
- Standart kullanıcı signup/login akışında Next.js yalnızca `AUTH_API_URL` ile yapılandırılan Go `services/auth-api` servisine proxy olur. Go servisi Neon Auth `sign-up/email` ve `sign-in/email` endpoint'lerini çağırır, doğrulanmış JWT içindeki `sub` ve `email` profil bilgilerini `app_user_profiles` içine upsert eder ve `USER_SESSION_SECRET` ile imzalı, httpOnly `koschei_member_session` cookie üretir. Uygulama parola hash'i saklamaz veya doğrulamaz. `/admin` erişimi tamamen ayrıdır ve yalnızca server-side `ADMIN_EMAIL` ile `ADMIN_PASSWORD` doğrulamasını kullanır.
