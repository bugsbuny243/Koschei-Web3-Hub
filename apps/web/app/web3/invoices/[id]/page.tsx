...
export default async function InvoiceDetail({params}:{params:{id:string}}){
  try {
    ...
    return <main className="mx-auto max-w-5xl p-6 space-y-4"><h1 className="text-2xl font-bold">Invoice {params.id}</h1><p className="rounded-lg bg-amber-100 p-3 text-sm">{warning}</p><pre className="bg-gray-100 p-3 rounded overflow-auto">{JSON.stringify(invoice.rows[0],null,2)}</pre><h2 className="font-semibold">Payment Events</h2><pre className="bg-gray-100 p-3 rounded overflow-auto">{JSON.stringify(events.rows,null,2)}</pre><h2 className="font-semibold">Accounting Entries</h2><pre className="bg-gray-100 p-3 rounded overflow-auto">{JSON.stringify(entries.rows,null,2)}</pre></main>;
  } catch (error) {
    return <main className="mx-auto max-w-5xl p-6 space-y-4"><h1 className="text-2xl font-bold">Invoice {params.id}</h1><p className="rounded-lg bg-amber-100 p-3 text-sm">{warning}</p><p className="rounded-lg bg-red-100 p-3 text-sm text-red-700">Failed to load invoice details: {error instanceof Error ? error.message : "Unknown database error"}</p></main>;
  }
}
