...
export function renderPreviewHtml(sceneConfig:SceneConfig){
  ...
  return buildPreviewHtml({
    ...
    prompt: b.victory,
    genre: b.template,
    visual_style: b.hud.join(" · "),
    labels: b.entities
  });
}

export function buildPreviewHtml(brief:Record<string,unknown>){/* compatibility wrapper */
  const b = brief as {template:TemplateKey;title:string;prompt?:string;genre?:string;visual_style?:string;labels?:string[]};
  const safeTitle = JSON.stringify(b.title ?? "Prompt Runner");
  const safePrompt = JSON.stringify(b.prompt ?? "");
  const safeGenre = JSON.stringify(b.genre ?? "");
  const safeVisualStyle = JSON.stringify(b.visual_style ?? "");
  const safeLabels = JSON.stringify(Array.isArray(b.labels) ? b.labels : []);
  const base = `<!doctype html><html><body style="margin:0;background:#070b16;color:#fff;font-family:system-ui"><canvas id=c width=760 height=420></canvas><script>const c=document.getElementById('c'),x=c.getContext('2d');const K={};onkeydown=e=>K[e.key]=1;onkeyup=e=>K[e.key]=0;let score=0,hp=5,tick=0;function txt(t,y){x.fillStyle='#fff';x.fillText(t,12,y);}function clamp(v,a,b){return Math.max(a,Math.min(b,v));}const TITLE=${safeTitle};const PROMPT=${safePrompt};const GENRE=${safeGenre};const VISUAL_STYLE=${safeVisualStyle};const LABELS=${safeLabels};`;
  const end = `requestAnimationFrame(loop);}loop();</script></body></html>`;
  if (b.template==="quest_arena_rpg") return `${base}let p={x:60,y:220,w:20,h:20},badges=[{x:180,y:90,t:0},{x:330,y:300,t:0},{x:520,y:180,t:0}],guards=[{x:420,y:90},{x:620,y:280}],chest={x:680,y:200,open:0};function loop(){tick++;x.fillStyle='#0d1326';x.fillRect(0,0,760,420);x.font='14px sans-serif';txt(TITLE,20);txt('Quest progress: Badges '+badges.filter(b=>b.t).length+'/3 HP '+hp+' Score '+score,40);if(K.ArrowUp)p.y-=3;if(K.ArrowDown)p.y+=3;if(K.ArrowLeft)p.x-=3;if(K.ArrowRight)p.x+=3;p.x=clamp(p.x,0,740);p.y=clamp(p.y,0,400);for(const g of guards){g.x+=Math.sin((tick+g.y)/30)*1.2;if(Math.abs(p.x-g.x)<16&&Math.abs(p.y-g.y)<16){hp--;p.x=60;p.y=220;}}for(const b of badges){if(!b.t&&Math.abs(p.x-b.x)<14&&Math.abs(p.y-b.y)<14){b.t=1;score+=15;}}if(badges.every(b=>b.t))chest.open=1;if(chest.open&&Math.abs(p.x-chest.x)<20&&Math.abs(p.y-chest.y)<20){txt('VICTORY: final chest unlocked',64);}x.fillStyle='#22d3ee';x.fillRect(p.x,p.y,20,20);x.fillStyle='#facc15';for(const b of badges){if(!b.t){x.beginPath();x.arc(b.x,b.y,8,0,6.28);x.fill();}}x.fillStyle='#ef4444';for(const g of guards){x.fillRect(g.x,g.y,18,18);}x.fillStyle=chest.open?'#86efac':'#6b7280';x.fillRect(chest.x,chest.y,26,20);if(hp<=0)txt('Game Over',64);${end}`;
  if (b.template==="boss_fight") return `${base}let p={x:120,y:210},boss={x:620,y:170,hp:240},orbs=[{x:300,y:120}],shots=[];function loop(){tick++;x.fillStyle='#0b1020';x.fillRect(0,0,760,420);txt(TITLE,20);txt('HP '+hp+' Score '+score+' Boss HP '+boss.hp,40);if(K.ArrowUp)p.y-=3;if(K.ArrowDown)p.y+=3;if(K.ArrowLeft)p.x-=3;if(K.ArrowRight)p.x+=3;p.x=clamp(p.x,0,740);p.y=clamp(p.y,0,400);if(tick%20===0)shots.push({x:p.x+18,y:p.y+10});for(const s of shots){s.x+=5;if(Math.abs(s.x-boss.x)<30&&Math.abs(s.y-boss.y)<50){boss.hp-=2;score+=20;}}if(tick%40===0){const hzY=40+Math.random()*340;if(Math.abs(p.y-hzY)<20)hp--;}for(const o of orbs){if(Math.abs(p.x-o.x)<14&&Math.abs(p.y-o.y)<14){hp=Math.min(5,hp+1);o.x=120+Math.random()*520;o.y=40+Math.random()*330;}}x.fillStyle='#22d3ee';x.fillRect(p.x,p.y,20,20);x.fillStyle='#a78bfa';x.fillRect(boss.x,boss.y,40,80);x.fillStyle='#f97316';for(const s of shots)x.fillRect(s.x,s.y,8,2);x.fillStyle='#fde047';for(const o of orbs){x.beginPath();x.arc(o.x,o.y,6,0,6.28);x.fill();}if(boss.hp<=0)txt('VICTORY',64);if(hp<=0)txt('Game Over',82);${end}`;
  if (b.template==="tower_defense") return `${base}let credits=100,base=20,wave=1,en=[{t:0},{t:120}],towers=[{x:260,y:120},{x:450,y:300}],slots=[{x:260,y:120},{x:450,y:300}];function loop(){tick++;x.fillStyle='#111827';x.fillRect(0,0,760,420);txt(TITLE,20);txt('Credits '+credits+' Base HP '+base+' Wave '+wave+' Score '+score,40);x.strokeStyle='#64748b';x.beginPath();x.moveTo(50,210);x.lineTo(700,210);x.stroke();for(const s of slots){x.strokeRect(s.x,s.y,20,20);}for(const t of towers){x.fillStyle='#60a5fa';x.fillRect(t.x,t.y,20,20);}for(const e of en){e.t+=0.7+wave*0.03;const ex=50+e.t,ey=210;x.fillStyle='#ef4444';x.fillRect(ex,ey,14,14);for(const t of towers){if(Math.abs(t.x-ex)<120&&tick%25===0){score+=10;e.t=0;credits+=5;}}if(ex>700){base--;e.t=0;}}if(tick%700===0){wave++;en.push({t:0});credits+=20;}if(base<=0)txt('Game Over',64);${end}`;
  if (b.template==="card_battle") return `${base}let php=30,ehp=30,turn=1,energy=3,msg='';c.onclick=(e)=>{const r=c.getBoundingClientRect(),x0=e.clientX-r.left,y0=e.clientY-r.top;if(y0>340&&y0<390){if(x0<230){ehp-=6;msg='Attack hit';}else if(x0<470){php=Math.min(30,php+4);msg='Shield up';}else if(energy>0){ehp-=10;energy--;msg='Blast fired';}if(ehp>0)php-=4;turn++;energy=Math.min(3,energy+1);}};function loop(){x.fillStyle='#1f2937';x.fillRect(0,0,760,420);txt(TITLE,20);txt('Player HP '+php+' Enemy HP '+ehp+' Turn '+turn+' Energy '+energy,40);x.fillStyle='#334155';x.fillRect(20,340,200,50);x.fillRect(260,340,200,50);x.fillRect(500,340,220,50);x.fillStyle='#fff';x.fillText('Attack',95,370);x.fillText('Shield',340,370);x.fillText('Energy Blast',565,370);txt(msg,70);if(ehp<=0)txt('VICTORY',90);if(php<=0)txt('Game Over',90);requestAnimationFrame(loop);}loop();</script></body></html>`;
  if (b.template==="resource_management") return `${base}let energy=20,minerals=10,shield=30,level=1,msg='';c.onclick=(e)=>{const r=c.getBoundingClientRect(),x0=e.clientX-r.left,y0=e.clientY-r.top;if(y0<330||y0>390)return;if(x0<250&&energy>=3){energy-=3;minerals+=5;msg='Mining surge';}else if(x0<500&&minerals>=8){minerals-=8;energy+=6;msg='Solar expanded';}else if(shield<40&&minerals>=5){minerals-=5;shield+=5;msg='Shield repaired';}if(minerals>30){level++;minerals-=12;msg='Colony level up';}};function loop(){if(tick++%120===0){energy++;minerals+=2;if(Math.random()<0.2){shield-=2;msg='Storm hit shield';}}x.fillStyle='#0f172a';x.fillRect(0,0,760,420);txt(TITLE,20);txt('Energy '+energy+' Minerals '+minerals+' Shield HP '+shield+' Colony Lv '+level,40);txt(msg,64);x.fillStyle='#334155';x.fillRect(20,330,220,60);x.fillRect(270,330,220,60);x.fillRect(520,330,220,60);x.fillStyle='#fff';x.fillText('Mine',105,365);x.fillText('Build Solar',335,365);x.fillText('Repair Shield',565,365);if(shield<=0)txt('Game Over',90);requestAnimationFrame(loop);}loop();</script></body></html>`;
  if (b.template==="arena_survival") return `${base}let p={x:380,y:210},wave=1,enemies=[{x:120,y:100},{x:640,y:300}],orbs=[{x:300,y:170}],xp=0;function spawnEnemy(){enemies.push({x:40+Math.random()*680,y:30+Math.random()*360});}function loop(){tick++;if(tick%500===0){wave++;spawnEnemy();spawnEnemy();}x.fillStyle='#111827';x.fillRect(0,0,760,420);txt(TITLE,20);txt('Wave '+wave+' HP '+hp+' Score '+score+' XP '+xp,40);if(K.ArrowUp)p.y-=2.8;if(K.ArrowDown)p.y+=2.8;if(K.ArrowLeft)p.x-=2.8;if(K.ArrowRight)p.x+=2.8;p.x=clamp(p.x,10,750);p.y=clamp(p.y,10,410);for(const e of enemies){const dx=p.x-e.x,dy=p.y-e.y,d=Math.hypot(dx,dy)||1;e.x+=dx/d*(1.1+wave*0.05);e.y+=dy/d*(1.1+wave*0.05);if(Math.abs(dx)<14&&Math.abs(dy)<14){hp--;e.x=20+Math.random()*720;e.y=20+Math.random()*380;}}for(const o of orbs){if(Math.abs(p.x-o.x)<14&&Math.abs(p.y-o.y)<14){xp++;score+=5;o.x=20+Math.random()*720;o.y=20+Math.random()*380;if(xp%4===0)spawnEnemy();}}if(tick%100===0){score+=wave;}x.fillStyle='#22d3ee';x.fillRect(p.x-10,p.y-10,20,20);x.fillStyle='#ef4444';for(const e of enemies)x.fillRect(e.x-8,e.y-8,16,16);x.fillStyle='#a3e635';for(const o of orbs){x.beginPath();x.arc(o.x,o.y,6,0,6.28);x.fill();}if(hp<=0)txt('Game Over',64);requestAnimationFrame(loop);}loop();</script></body></html>`;
  if (b.template==="platformer") return `${base}let p={x:70,y:250,w:18,h:22,vx:0,vy:0,on:0},plats=[{x:0,y:350,w:760,h:70},{x:130,y:290,w:120,h:14},{x:300,y:250,w:130,h:14},{x:500,y:210,w:120,h:14}],crystals=[{x:170,y:265,t:0},{x:350,y:225,t:0},{x:550,y:185,t:0}],spikes=[{x:250,y:338,w:40},{x:460,y:338,w:45}],portal={x:690,y:170,w:30,h:40};function overlap(a,b){return a.x<b.x+b.w&&a.x+a.w>b.x&&a.y<b.y+b.h&&a.y+a.h>b.y;}function loop(){tick++;x.fillStyle='#0f172a';x.fillRect(0,0,760,420);txt(TITLE,20);txt('HP '+hp+' Score '+score+' Crystals '+crystals.filter(c=>c.t).length+'/'+crystals.length,40);p.vx=(K.ArrowRight?2.5:0)-(K.ArrowLeft?2.5:0);if((K[' ']||K.ArrowUp)&&p.on){p.vy=-7.5;p.on=0;}p.vy+=0.35;p.x+=p.vx;p.y+=p.vy;p.on=0;for(const pl of plats){if(p.x+p.w>pl.x&&p.x<pl.x+pl.w&&p.y+p.h>=pl.y&&p.y+p.h<=pl.y+12&&p.vy>=0){p.y=pl.y-p.h;p.vy=0;p.on=1;}}for(const s of spikes){if(overlap(p,{x:s.x,y:332,w:s.w,h:18})){hp--;p.x=70;p.y=250;p.vy=0;}}for(const c0 of crystals){if(!c0.t&&Math.abs((p.x+9)-c0.x)<12&&Math.abs((p.y+11)-c0.y)<12){c0.t=1;score+=10;}}if(crystals.every(c=>c.t)&&overlap(p,portal)){txt('VICTORY: portal reached',64);}if(p.y>420){hp--;p.x=70;p.y=250;p.vy=0;}x.fillStyle='#38bdf8';x.fillRect(p.x,p.y,p.w,p.h);x.fillStyle='#334155';for(const pl of plats)x.fillRect(pl.x,pl.y,pl.w,pl.h);x.fillStyle='#ef4444';for(const s of spikes)x.fillRect(s.x,332,s.w,18);x.fillStyle='#a78bfa';x.fillRect(portal.x,portal.y,portal.w,portal.h);x.fillStyle='#fde047';for(const c0 of crystals){if(!c0.t){x.beginPath();x.arc(c0.x,c0.y,6,0,6.28);x.fill();}}if(hp<=0)txt('Game Over',86);requestAnimationFrame(loop);}loop();</script></body></html>`;
  return `${base}let p={x:90,y:270,w:18,h:18},stars=[{x:300,y:260},{x:480,y:220}],spikes=[{x:760,y:300,w:18}],speed=2.5;function loop(){tick++;x.fillStyle='#0b1220';x.fillRect(0,0,760,420);txt(TITLE,20);txt('Runner HP '+hp+' Score '+score,40);if(K.ArrowUp&&p.y>=270)p.y-=70;p.y+=2;if(p.y>270)p.y=270;for(const st of stars){st.x-=speed;if(st.x<-20)st.x=780;if(Math.abs(p.x-st.x)<14&&Math.abs(p.y-st.y)<14){score+=10;st.x=780;st.y=220+Math.random()*80;}}for(const s of spikes){s.x-=speed+1.2;if(s.x<-30)s.x=780+Math.random()*120;if(p.x+p.w>s.x&&p.x<s.x+s.w&&p.y+p.h>300){hp--;s.x=790;}}if(tick%240===0)speed+=0.2;x.fillStyle='#22d3ee';x.fillRect(p.x,p.y,p.w,p.h);x.fillStyle='#facc15';for(const st of stars){x.beginPath();x.arc(st.x,st.y,6,0,6.28);x.fill();}x.fillStyle='#ef4444';for(const s of spikes)x.fillRect(s.x,300,s.w,24);if(hp<=0)txt('Game Over',64);requestAnimationFrame(loop);}loop();</script></body></html>`;
}
